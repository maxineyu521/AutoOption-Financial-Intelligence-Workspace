...
def run_health_check():
    ...
    # 2. 抽查一条最新的 SEC 数据 (验证 Payload 是否健康)
    ...
    scroll_results, _ = client.scroll(
        ...
        limit=1,
        with_payload=True,
        with_vectors=True # 强行把向量拉下来看看
    )
    
    if scroll_results:
        doc = scroll_results[0]
        print(f"\n[Document ID]: {doc.id}")
        print(f"[Vector Types Found]: {list(doc.vector.keys())}") # 应该打印出 ['dense', 'sparse']
